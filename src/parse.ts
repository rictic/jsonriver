...
class Parser implements AsyncIterableIterator<JsonValue>, TokenHandler {
  ...
  handleStringStart(): void {
    ...
    switch (state.type) {
      case StateEnum.Initial:
        this.#stateStack.pop();
        this.#toplevelValue = this.#progressValue(JsonTokenType.StringStart, undefined);
        break;
  ...
  #handleValueToken(type: JsonTokenType, value: unknown): void {
    const state = this.#currentState();
    if (!this.#progressed) {
      this.#progressed = true;
    }
  ...
  #updateStringParent(updated: string, parentState: State | undefined): void {
